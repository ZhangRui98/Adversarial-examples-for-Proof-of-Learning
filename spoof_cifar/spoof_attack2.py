    ...
    for diff_k in different_t_settings:
        ...
        for test_times in range(experiment_times):
            ...
            if args.dataset == "CIFAR10":
                ...
                test_data = datasets.CIFAR100("data/cifar100", train=False, download=True, transform=transform_apply)
                test_loader = DataLoader(dataset=test_data, batch_size=args.batchsize, shuffle=False)
            ...
            state = torch.load("spoof_cifar/proof/{}/model_step_0".format(args.dataset),map_location={'cuda:2':'cuda:0'})
            net_load.load_state_dict(state['net'])
            # before train parameter
            bt_param = list((_.detach().clone() for _ in net_load.parameters()))

            state = torch.load("spoof_cifar/proof/{}/model_step_".format(args.dataset) + str(args.iter),map_location={'cuda:1':'cuda:0'})
            net_load.load_state_dict(state['net'])
            ...
            for i in range(args.t):
                ...
                for j in range(args.k):
                    ...
                    for k in range(args.retry):
                        ...
                        optimizer = torch.optim.LBFGS([dummy_data, ], max_iter=10)
                        ...
                        for iters in range(args.round):
                            ...
                            def closure():
                                ...
                                grad_diff = 0
                                for gx, gy in zip(dummy_dy_dx, step_dy_dx):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--t', type=int, default=15, help = "t from W0' to Wn")
    ...
    parser.add_argument('--retry', type=int, default=10, help = "retry times")
    parser.add_argument('--gd', type=float, default=30, help = "grad diff")
    parser.add_argument('--nd', type=int, default=30, help = "noise diff")
